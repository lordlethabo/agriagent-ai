...
- Maximum 80 words.
- Be direct, practical, and measurable.
- Focus on global challenges: food security, climate resilience, water scarcity, poverty reduction.
""",
...
def planner_agent(client, deployment, farmer_data):
    return call_agent(
        client, deployment, "Planner Agent", farmer_data,
...
def water_agent(client, deployment, farmer_data):
    return call_agent(
        client, deployment, "Water Agent", farmer_data,
...
def risk_agent(client, deployment, farmer_data):
    return call_agent(
        client, deployment, "Risk Agent", farmer_data,
        "Identify climate, pest, financial, and market risks with mitigation actions."
    )
...
Rules:
- Keep it short.
- Explain each score in 1 sentence.
